...
int main(int argc, char *argv[]) {
  ...
  aa_filterbank_metadata filterbank_metadata = filterbank_datafile.read_metadata(ddtr_plan.user_dm(ddtr_plan.range()-1).high);

  if(!filterbank_datafile.read_signal()) {
    LOG(log_level::error, "Could not read telescope dataa.");
    return 0;
  }
