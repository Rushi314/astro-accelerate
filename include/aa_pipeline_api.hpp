...
//          if (m_filterbank_metadata.strat() == 0) {
          printf(" \n aa_pipeline_api.hpp :: m_filterbank_metadata.strat(): %d \n", m_filterbank_metadata.strat());
	  printf("\n aa_pipeline_api.hpp :: m_ddtr_strategy.maxshift() ::  %d \n", m_ddtr_strategy.maxshift());
	  aa_ddtr_strategy ddtr_strategy(m_ddtr_plan, m_filterbank_metadata, m_card_info.free_memory, false);
	  printf("\n ddtr_strategy.ready(): %d \n", ddtr_strategy.ready());
	  printf("\n aa_pipeline_api.hpp :: we are hereeeee \n");
	  printf("\n aa_pipeline_api.hpp :: m_filterbank_metadata.nchans(): %d\n", m_filterbank_metadata.nchans());
	  if(ddtr_strategy.ready()) {
	    m_ddtr_strategy = std::move(ddtr_strategy);
	    m_all_strategy.push_back(&m_ddtr_strategy);
	    //If the plan is valid then the supplied_plan becomes true
...
/*          } // strat if condition ends here ---------------------------------------------------------------------------
          else {
            printf("\n else condition in aa_pipeline_api.hpp :: bool bind \n");
	    supplied_plans.at(aa_pipeline::component::dedispersion) = true;
          } // strat else condition ends here -------------------------------------------------------------------------
*/	}
      }
      else {
    ...
    bool run() {
       ...
       */
      printf(" \n 1. aa_pipeline_api.hpp :: m_filterbank_metadata.strat(): %d \n", m_filterbank_metadata.strat());
      unsigned long int f_pos;

      if(pipeline_ready && m_runner->setup()) {
//      if((pipeline_ready && m_runner->setup()) || m_filterbank_metadata.strat()) {
        int max_iter = 3;
        for (int buf_count = 0; buf_count < max_iter; buf_count++)
        {
...
//        aa_sigproc_input       filterbank_datafile("/data/jroy/data/FRB_DM1000_163.84us_4K_7mP_4msW_3sig.header.fil");
        ...
        if(!filterbank_datafile.read_new_signal(buf_count, m_filterbank_metadata, &f_pos)) {
            std::cout << "ERRORRR: Could not read telescope data." << std::endl;
            }

        filterbank_datafile.close();
//        } if buf_count condition ends here -------------------------------------------------


	while(m_runner->next()) {
	  LOG(log_level::notice, "Pipeline running over next chunk.");
	}
        printf("\n one buf_count loop ends here -----------------------------next begins ------ \n");
	...
	LOG(log_level::error, " 1. Pipeline could not start/resume because either pipeline is not ready or runner is not setup.");
	return false;
      }
	...
	LOG(log_level::error, " 2. Pipeline could not start/resume because either pipeline is not ready or runner is not setup.");
